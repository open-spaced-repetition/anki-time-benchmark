...
6) fsrs_r_linear_robust: same as above, but `a` and `b` are fitted using the [Siegel estimator](https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.siegelslopes.html) instead of ordinary least squares. This makes it robust to outliers.
7) fsrs_r_grade_interact: first review -> first-grade median, else `t=a0+a1*G+a2*R+a3*G*R`, where G (grade) can take values 1, 2, 3, 4 for Again, Hard, Good and Easy respectively. `a0`, `a1`, `a2` and `a3` are estimated based on each user's review history.
